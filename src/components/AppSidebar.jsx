...
export function AppSidebar() {
  ...
  useEffect(() => {
    // Simplified animations that don't affect layout
    const ctx = gsap.context(() => {
      ...
      // Animate menu items without affecting layout
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-b from-slate-900 via-slate-900 to-slate-800 border-r border-cyan-500/20 relative overflow-hidden">
      ...
      <div className="border-b border-cyan-500/20 p-6 relative z-10 flex-shrink-0">
        <div ref={logoRef} className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-xl text-white mb-0.5">OceanVista</h2>
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
        <div className="flex-1 flex flex-col">
          <div className="text-cyan-400/60 text-xs font-semibold uppercase tracking-wider px-6 py-4 flex-shrink-0">
            Navigation
          </div>
          <div className="flex-1 px-3 overflow-y-auto">
            <div ref={menuRef} className="space-y-1 pb-4">
              {menuItems.map((item, index) => {
                const isActive = location.pathname === item.url;
                return (
                  <div key={item.title} data-menu-item>
                    <NavLink
                      to={item.url}
                      className={`
